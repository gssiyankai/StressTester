        ...
        try (FileWriter fw = new FileWriter(file);
            ...
            for (BatchResult result : results) {
                ...
                for (TimestampedMessage request : requests) {
                    out.append(messageToRow(result.batchId(), result.runId(), "request", request, separator));
                    out.append("\n");
                }
                List<TimestampedMessage> responses = result.responses();
                for (TimestampedMessage response : responses) {
                    out.append(messageToRow(result.batchId(), result.runId(), "response", response, separator));
                    out.append("\n");
                }
            }
        }
